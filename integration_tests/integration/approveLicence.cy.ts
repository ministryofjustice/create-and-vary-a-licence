...
context('Approve a licence', () => {
  ...
  it('should display HDC content if licence kind is HDC', () => {
    ...
    approvalViewPage.getValue(approvalViewPage.homeDetentionCurfewActualDate).should('contain.text', '9 September 2023')
    cy.contains(approvalViewPage.homeDetentionCurfewActualDate, 'CALCULATE RELEASE DATE')
    approvalViewPage
      .getValue(approvalViewPage.curfewAddress)
      .should('contain.text', '1 The Street, Avenue, Some Town, Some County, A1 2BC')
    cy.contains(approvalViewPage.curfewAddress, 'HDC')

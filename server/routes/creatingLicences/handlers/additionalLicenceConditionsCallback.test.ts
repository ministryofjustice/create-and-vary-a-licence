...
describe('Route Handlers - Create Licence - Additional Conditions Callback', () => {
  ...
  describe('GET', () => {
    it('should redirect to the input page for the first additional condition in sequence which requires input but doesnt have any yet', async () => {
      ...
      conditionsProviderSpy.mockReturnValueOnce(Promise.resolve({ text: 'Condition 1', requiresInput: true, code: 'CON1' }))
      conditionsProviderSpy.mockReturnValueOnce(Promise.resolve({ text: 'Condition 2', requiresInput: false, code: 'CON2' }))
      conditionsProviderSpy.mockReturnValueOnce(Promise.resolve({ text: 'Condition 3', requiresInput: true, code: 'CON3' }))
      conditionsProviderSpy.mockReturnValueOnce(Promise.resolve({ text: 'Condition 4', requiresInput: true, code: 'CON4' }))

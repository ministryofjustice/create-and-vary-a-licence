...
describe('Create a Licence Views - Check Answers', () => {
  ...
  it('should not display the HDC migration warning message above bespoke conditions for variations of HDC migrations when there are no bespoke conditions', () => {
    const $ = render({
      licence: { ...licence, typeCode: 'AP', bespokeConditions: [] },
      isVariationOfHdcMigration: true,
    })
    expect($('#hdc-migration-bespoke-conditions-warning').length).toBe(0)
  })
